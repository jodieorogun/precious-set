...
export function OpeningHoursSettings() {
  ...
  useEffect(() => {
    async function loadHours() {
      if (!supabase) return;
      const { data } = await supabase.from("opening_hours").select("id, dayOfWeek, isOpen, openTime, closeTime, breakStart, breakEnd").order("dayOfWeek");
      setHours((data ?? []) as OpeningHour[]);
    }
    void loadHours();
  }, []);

  function updateHour(id: string, changes: Partial<OpeningHour>) {
  ...
  return (
    <details className="hours-settings">
      <summary><span><small>Availability</small><strong>Opening hours</strong></span><span className="hours-toggle">⌄</span></summary>
      <div className="hours-settings-content">
        <div className="hours-settings-actions"><p>Set when customers can request appointments.</p><button className="button approve-button" onClick={() => void saveHours()}>Save hours</button></div>
        <div className="hours-list">
          {hours.map((hour) => (
            <div className={`hours-row${hour.isOpen ? "" : " is-closed"}`} key={hour.id}>
              <label className="hours-day"><input type="checkbox" checked={hour.isOpen} onChange={(event) => updateHour(hour.id, { isOpen: event.target.checked })} /><span>{dayNames[hour.dayOfWeek]}</span></label>
              {hour.isOpen ? (
                <>
        ...
        {message && <p className="hours-message">{message}</p>}
      </div>
    </details>
  );
}
