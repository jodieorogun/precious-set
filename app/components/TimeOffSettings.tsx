...
export function TimeOffSettings() {
  ...
  async function loadPeriods() {
    if (!supabase) return;
    const { data } = await supabase.from("blocked_periods").select("id, startDate, endDate, reason").order("startDate");
  ...
  async function addPeriod(event: FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.from("blocked_periods").insert({ startDate, endDate, reason: reason.trim() || null });
    if (error) { setMessage(error.message); return; }
    setStartDate(""); setEndDate(""); setReason(""); setMessage("Time off added."); await loadPeriods();
  }
  ...
  return <details className="time-off-settings"><summary><span><small>Availability</small><strong>Time off</strong></span><span className="hours-toggle">⌄</span></summary><div className="time-off-content"><p className="time-off-intro">Block dates when the studio is unavailable, such as holidays.</p><form className="time-off-form" onSubmit={addPeriod}><label>From<input required type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /></label><label>To<input required type="date" min={startDate || undefined} value={endDate} onChange={(event) => setEndDate(event.target.value)} /></label><label>Reason <span className="optional">optional</span><input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Holiday" /></label><button className="button approve-button" type="submit">Add time off</button></form>{periods.length > 0 && <div className="time-off-list">{periods.map((period) => <div className="time-off-row" key={period.id}><span><strong>{formatDate(period.startDate)} – {formatDate(period.endDate)}</strong><small>{period.reason || "Studio unavailable"}</small></span><button className="text-button" type="button" onClick={() => void removePeriod(period.id)}>Remove</button></div>)}</div>}{message && <p className="hours-message">{message}</p>}</div></details>;
}
