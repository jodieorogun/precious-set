...
export function BookingForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setErrorMessage(""); if (!supabase) { setErrorMessage("Booking is not connected yet. Add your Supabase environment variables first."); return; } if (!phoneVerified) { setErrorMessage("Please verify your phone number before sending a booking request."); return; } if (!selectedService) { setErrorMessage("Please choose a service."); return; } if (!openingHour?.isOpen) { setErrorMessage("The studio is closed on that date. Please choose another day."); return; } if (!startTime) { setErrorMessage("Please choose an available time."); return; } setIsSubmitting(true); const { data: userData } = await supabase.auth.getUser(); if (!userData.user) { setIsSubmitting(false); setErrorMessage("Please verify your phone number before sending a booking request."); return; } const { error } = await supabase.from("bookings").insert({ customerUserId: userData.user.id, customerName, customerPhone, serviceId, bookingDate, startTime, endTime: getEndTime(startTime, selectedService.durationMinutes), notes: notes || null, status: "pending" }); setIsSubmitting(false); if (error) { setErrorMessage(error.message); return; } router.push("/book/confirmation"); }
