...
export function BookingForm() {
  ...
  return <form className="booking-form" onSubmit={handleSubmit}>
    ...
    </div><div className="booking-overview"><p className="eyebrow">Booking overview</p>{selectedService ? <><div className="booking-overview-line"><span>{selectedService.name}{addOns.length ? ` + ${addOns.join(" + ")}` : ""}</span><strong>{bookingDate && startTime ? `${selectedDateLabel} · ${startTime}–${getEndTime(startTime, bookingDuration)}` : "Choose a date and time"}</strong></div><div className="booking-overview-line"><span>Estimated duration</span><strong>{Math.floor(bookingDuration / 60)} hr{bookingDuration % 60 ? ` ${bookingDuration % 60} mins` : ""}</strong></div><div className="booking-overview-line"><span>Estimated price</span><strong>£{(Number(selectedService.startingPrice) + (addOns.includes("Removal / Soak Off") ? 10 : 0)).toFixed(2)}{addOns.includes("Nail Art & Charms") ? "–£" + (Number(selectedService.startingPrice) + (addOns.includes("Removal / Soak Off") ? 10 : 0) + 12).toFixed(2) + " (depending on design difficulty)" : ""}</strong></div></> : <p className="picker-placeholder">Choose a service to see your booking overview.</p>}</div></div>
    <div className="form-section"><p className="eyebrow">02 / Your details</p><div className="form-grid"><label>Name<input required value={customerName} onChange={(event) => setCustomerName(event.target.value)} placeholder="Your name" /></label><label>Phone number<input required type="tel" inputMode="tel" value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} placeholder="07…" /></label><div className="full-width addon-picker"><span className="field-label">Extras <span className="optional">optional</span></span><label className="addon-option"><input type="checkbox" checked={addOns.includes("Removal / Soak Off")} onChange={(event) => setAddOns((current) => event.target.checked ? [...current, "Removal / Soak Off"] : current.filter((item) => item !== "Removal / Soak Off"))} /><span><strong>Removal / Soak Off</strong><small>+30 minutes · £10</small></span></label><label className="addon-option"><input type="checkbox" checked={addOns.includes("Nail Art & Charms")} onChange={(event) => setAddOns((current) => event.target.checked ? [...current, "Nail Art & Charms"] : current.filter((item) => item !== "Nail Art & Charms"))} /><span><strong>Nail Art & Charms</strong><small>+30 minutes · £5–£12 depending on design difficulty</small></span></label></div><label className="full-width">Notes <span className="optional">optional</span><textarea rows={4} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Anything The Precious Set should know?" /></label></div></div>
