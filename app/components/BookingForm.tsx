...
export function BookingForm() {
  ...
  return <form className="booking-form" onSubmit={handleSubmit}>
    ...
    <div className="form-section"><p className="eyebrow">02 / Your details</p><div className="form-grid"><label>Name<input required value={customerName} onChange={(event) => setCustomerName(event.target.value)} placeholder="Your name" /></label><label>Phone number<input required type="tel" inputMode="tel" value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} placeholder="07…" /></label><div className="full-width addon-picker"><span className="field-label">Extras <span className="optional">optional</span></span><label className="addon-option"><input type="checkbox" checked={addOns.includes("Painting")} onChange={(event) => setAddOns((current) => event.target.checked ? [...current, "Painting"] : current.filter((item) => item !== "Painting"))} /><span><strong>Painting</strong><small>Included · +20 minutes</small></span></label><label className="addon-option"><input type="checkbox" checked={addOns.includes("Nail Art & Charms")} onChange={(event) => setAddOns((current) => event.target.checked ? [...current, "Nail Art & Charms"] : current.filter((item) => item !== "Nail Art & Charms"))} /><span><strong>Nail Art & Charms</strong><small>+30 minutes · £5–£12</small></span></label></div><label className="full-width">Notes <span className="optional">optional</span><textarea rows={4} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Anything The Precious Set should know?" /></label></div></div>
