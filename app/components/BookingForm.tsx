...
export function BookingForm() {
  ...
  const [customerName, setCustomerName] = useState(""); const [customerPhone, setCustomerPhone] = useState(""); const [verificationCode, setVerificationCode] = useState(""); const [phoneCodeSent, setPhoneCodeSent] = useState(false); const [phoneVerified, setPhoneVerified] = useState(false); const [notes, setNotes] = useState("");
  ...
  useEffect(() => {
    async function loadMonthAvailability() {
      ...
      const results = await Promise.all(days.map(async (date) => {
        ...
        return [key, getTimeOptions(hours.openTime, hours.closeTime, hours.breakStart, hours.breakEnd, selectedService.durationMinutes, (data ?? []) as BusySlot[]).length > 0] as const;
      }));
      setMonthAvailability(Object.fromEntries(results));
    }
    void loadMonthAvailability();
  }, [calendarMonth, selectedService, openingHours, today]);

  async function sendPhoneCode() { if (!supabase || !customerPhone) { setErrorMessage("Enter your phone number first."); return; } setErrorMessage(""); const { error } = await supabase.auth.signInWithOtp({ phone: customerPhone }); if (error) { setErrorMessage(error.message); return; } setPhoneCodeSent(true); }
  async function verifyPhoneCode() { if (!supabase || !verificationCode) return; setErrorMessage(""); const { data, error } = await supabase.auth.verifyOtp({ phone: customerPhone, token: verificationCode, type: "sms" }); if (error || !data.user) { setErrorMessage(error?.message ?? "That code could not be verified."); return; } setPhoneVerified(true); }

  async function handleSubmit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setErrorMessage(""); if (!supabase) { setErrorMessage("Booking is not connected yet. Add your Supabase environment variables first."); return; } if (!phoneVerified) { setErrorMessage("Please verify your phone number before sending a booking request."); return; } if (!selectedService) { setErrorMessage("Please choose a service."); return; } if (!openingHour?.isOpen) { setErrorMessage("The studio is closed on that date. Please choose another day."); return; } if (!startTime) { setErrorMessage("Please choose an available time."); return; } setIsSubmitting(true); const { data: userData } = await supabase.auth.getUser(); if (!userData.user) { setIsSubmitting(false); setErrorMessage("Please verify your phone number before sending a booking request."); return; } const manageToken = crypto.randomUUID(); const manageTokenHash = await hashManageToken(manageToken); const { error } = await supabase.from("bookings").insert({ customerUserId: userData.user.id, customerName, customerPhone, serviceId, bookingDate, startTime, endTime: getEndTime(startTime, selectedService.durationMinutes), notes: notes || null, manageTokenHash, status: "pending" }); setIsSubmitting(false); if (error) { setErrorMessage(error.message); return; } router.push(`/book/confirmation?token=${encodeURIComponent(manageToken)}`); }
  ...
  return <form className="booking-form" onSubmit={handleSubmit}>
    ...
    <div className="form-section"><p className="eyebrow">02 / Your details</p><div className="form-grid"><label>Name<input required value={customerName} onChange={(event) => setCustomerName(event.target.value)} placeholder="Your name" /></label><div className="phone-auth-field"><label>Phone number<input required type="tel" inputMode="tel" value={customerPhone} onChange={(event) => { setCustomerPhone(event.target.value); setPhoneVerified(false); setPhoneCodeSent(false); }} placeholder="07…" /></label><button type="button" className="text-button phone-code-button" onClick={() => void sendPhoneCode()} disabled={phoneVerified}>{phoneVerified ? "Phone verified" : "Send verification code"}</button>{phoneCodeSent && !phoneVerified && <div className="phone-code-row"><input aria-label="SMS verification code" inputMode="numeric" value={verificationCode} onChange={(event) => setVerificationCode(event.target.value)} placeholder="Enter code" /><button type="button" className="button approve-button" onClick={() => void verifyPhoneCode()}>Verify</button></div>}</div><label className="full-width">Notes <span className="optional">optional</span><textarea rows={4} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Anything The Precious Set should know?" /></label></div></div>
