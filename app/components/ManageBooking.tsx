...
export function ManageBooking() {
  ...
  return <section className="manage-booking-page page-shell"><p className="eyebrow">Manage booking</p><h1>{booking.serviceName}</h1><div className="manage-booking-card"><div><span className="detail-label">Date</span><strong>{formatDate(booking.bookingDate)}</strong></div><div><span className="detail-label">Time</span><strong>{booking.startTime.slice(0, 5)}–{booking.endTime.slice(0, 5)}</strong></div><div><span className="detail-label">Price</span><strong>£{Number(booking.startingPrice).toFixed(2)}</strong></div><div><span className="detail-label">Status</span><strong className="status-pill">{booking.status}</strong></div></div>{canCancel ? <><p className="manage-booking-policy">Need to cancel? You can cancel online if there are at least 24 hours before your appointment.</p><button className="button decline-button" onClick={() => void cancelBooking()} disabled={isCancelling}>{isCancelling ? "Cancelling…" : "Cancel appointment"}</button></> : <p className="form-error">This appointment can no longer be cancelled online. Please contact The Precious Set directly.</p>}{errorMessage && <p className="form-error">{errorMessage}</p>}<p><Link className="text-link" href="/">Back to The Precious Set</Link></p></section>;
}
