...
type Booking = { id: string; customerUserId: string | null; customerName: string; customerPhone: string; customerEmail: string | null; serviceId: string; bookingDate: string; startTime: string; endTime: string; notes: string | null; addOns: string[] | null; addonPrice: number | null; status: string; services: { name: string; startingPrice: number } | null; };
...
export function AdminDashboard() {
  ...
  async function loadBookings() {
    ...
    const [{ data: bookingData, error: bookingError }, { data: serviceData }] = await Promise.all([supabase.from("bookings").select("id, customerUserId, customerName, customerPhone, serviceId, bookingDate, startTime, endTime, notes, addOns, addonPrice, status, services(name, startingPrice)").order("bookingDate", { ascending: true }).order("startTime", { ascending: true }), supabase.from("services").select("id, name, durationMinutes, startingPrice").eq("isActive", true).order("name")]);
