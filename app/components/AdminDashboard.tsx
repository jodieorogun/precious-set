...
export function AdminDashboard() {
  ...
  async function updateBooking(id: string, status: "confirmed" | "declined") { if (!supabase) return; const { error } = await supabase.from("bookings").update({ status }).eq("id", id); if (error) setErrorMessage(error.message); else setBookings((current) => current.map((booking) => booking.id === id ? { ...booking, status } : booking)); }
