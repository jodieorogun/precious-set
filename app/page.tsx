...
export default function Home() {
  return <main className="creative-home">
    <section className="creative-hero page-shell">
      <div className="creative-hero-copy"><h1>the<br /><em>precious</em><br />set</h1><Link className="creative-book" href="/book">Book an appointment <span>↗</span></Link></div>
    ...
    <section className="creative-details page-shell" aria-label="The Precious Set information">{studioDetails.map(([number, title, description, href, action]) => href.startsWith("http") ? <a className={`creative-detail creative-detail-${number}`} href={href} target="_blank" rel="noreferrer" key={number}><strong>{title}</strong><p>{description}</p><i>{action} ↗</i></a> : <Link className={`creative-detail creative-detail-${number}`} href={href} key={number}><strong>{title}</strong><p>{description}</p><i>{action} {title === "Find us" ? "↓" : "↗"}</i></Link>)}<div className="creative-detail creative-location-panel"><strong>Location</strong><p>Leeds, United Kingdom</p><i aria-hidden="true">&nbsp;</i></div></section>
  </main>;
}
